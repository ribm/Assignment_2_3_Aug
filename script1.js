...
function eject(Countries)
{
    ...
    button.setAttribute("onclick", "temp1(this.id)");
...
async function temp1(id){

    try{
        var response = await fetch('https://api.openweathermap.org/data/2.5/weather?q='+id+'&appid=8268b17f393fe7bfcc03f70e2f6d2f98')
        var result = await response.json()
        alert("the temp in capital.."+id+" "+result.main.temp)
    }
    catch(error){
        alert("Error in name ")
    }
